  ...
  suite("Routing tests", function () {
    ...
    suite("GET /api/books/[id] => book object with [id]", function () {
      test("Test GET /api/books/[id] with id not in db", function (done) {
        chai
          .request(server)
          .get("/api/books/63657911922d375e25ad1c45")
          .end((err, res) => {
            assert.equal(res.status, 200);
            assert.isString(res.body);
      ...
      test("Test GET /api/books/[id] with valid id in db", function (done) {
        chai
          ...
          .end((err, res) => {
            ...
            assert.equal(res.body.title, "Personal Library");
      ...
      function () {
        test("Test POST /api/books/[id] with comment", function (done) {
          chai
            ...
            .send({ comment: "testlib" })
            .end((err, res) => {
              assert.equal(res.status, 200);
              assert.isObject(res.body);
              assert.equal(res.body._id, "63680748dad31302a987eaed");
              assert.equal(res.body.title, "Comment modification testlib");
              assert.isArray(res.body.comments);
              assert.equal(res.body.comments[0], "testlib");
              done();
            });
        });
        ...
        test("Test POST /api/books/[id] with comment, id not in db", function (done) {
          chai
            .request(server)
            .post("/api/books/63680748dad31302a987eae2")
            .send({ comment: "bla" })
            .end((err, res) => {
              assert.equal(res.status, 200);
              assert.isString(res.body);
    ...
    suite("DELETE /api/books/[id] => delete book object id", function () {
      ...
      test("Test DELETE /api/books/[id] with  id not in db", function (done) {
        chai
          .request(server)
          .delete("/api/books/63657911922d375e25ad1b81")
          .end((err, res) => {
            assert.equal(res.status, 200);
            assert.isString(res.body);
            assert.equal(res.body, "no book exists");
